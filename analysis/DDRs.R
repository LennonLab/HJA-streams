...
water.geo.dist.ls <- na.omit(liste(
  dist.mat[which(design$habitat == "water"), 
           which(design$habitat == "water")],
  entry = "geo.dist")[,3])
...
sed.geo.dist.ls <- na.omit(
  liste(dist.mat[which(design$habitat == "sediment"),
                 which(design$habitat == "sediment")], 
        entry = "geo.dist")[,3])
...
headwater.geo.dists <- na.omit(
  liste(dist.mat[which(design$order < 2), which(design$order < 2)],
        entry = "geo.dist")[,3])
headwater.db <- vegdist(OTUsREL.log[which(design$order < 2),], 
                        method = dist.met)
...
headwater.dists <- data.frame(
  liste((1 - headwater.db), entry = "comm.struc")[,3],
...
downstream.db <- vegdist(OTUsREL.log[which(design$order >= 2),],
                         method = dist.met)
...
downstream.dists <- data.frame(
  liste((1 - downstream.db), entry = "comm.struc")[,3],
...
downstream.sed.geo.dists <- na.omit(
  liste(dist.mat[which(design$order >1  & design$habitat == "sediment"),
...
downstream.sed.dists <- data.frame(
  liste((1 - downstream.sed.db), entry = "comm.struc")[,3],
...
downstream.water.geo.dists <- na.omit(
  liste(dist.mat[which(design$order >1  & design$habitat == "water"),
                 which(design$order >1 & design$habitat == "water")],
        entry = "geo.dist"))[,3]
downstream.water.db <- vegdist(
...
downstream.water.dists <- data.frame(
  liste((1 - downstream.water.db), entry = "comm.struc")[,3], 
