...
water.distances <- data.frame(water.coord.dist.ls, water.struc.dist.ls[,3])
...
sed.coord.dist <- dist(as.matrix(sed.xy))
...
plot(water.distances$dist, water.distances$struc, xlab="", 
     ylab = "", xaxt="n", yaxt="n", 
     xlim = c(0,12500), ylim = c(0,.6))
(water.lm <- summary(lm(water.distances$struc ~ water.distances$dist)))
abline(water.lm)
axis(side=1, labels=F, lwd.ticks=2, cex.axis=1.2, las=1)
axis(side=2, labels=T, lwd.ticks=2, cex.axis=1.2, las=1)
axis(side=3, labels=F, lwd.ticks=2, cex.axis=1.2, las=1)
...
box(lwd=2)
mtext("Sediment", side = 2, line = 3, cex = 1.2)
sed.lm <- summary(lm(sed.distances$struc ~ sed.distances$dist))
abline(sed.lm)
