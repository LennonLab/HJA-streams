...
boxplot(dist ~ order, data = orders.bray.s)
```

### Figure 4: Beta-diversity is highest among headwater streams
```{r, eval=F}
png(filename = "./figures/Figure4.png",
    width = 1200, height = 1200, res = 96*2)
par(mar = c(5, 5, 2, 2) + 0.1)
boxplot(dist ~ order, data = orders.bray.w, at = c(1,4,7), xlim = c(0,9), ylim = c(0.4, 0.9), 
        col = "skyblue", xlab = "", xaxt = "n", yaxt = "n")
boxplot(dist ~ order, data = orders.bray.s, at = c(2,5,8), add = T, col = "wheat",
        xlab = "", xaxt = "n", yaxt = "n")
axis(side = 1, labels = F, at = c(1.5, 4.5, 7.5), lwd.ticks = 2, cex.axis = 1.2, las = 1)
axis(side = 2, labels = T, lwd.ticks = 2, cex.axis = 1.2, las = 1)
axis(side = 3, labels = F, lwd.ticks = 2, cex.axis = 1.2, las = 1)
axis(side = 4, labels = F, lwd.ticks = 2, cex.axis = 1.2, las = 1)
box(lwd=2)
...
mtext(side = 1, at = c(1.5,4.5,7.5), line = 1, c("1","2","3"), cex = 1.2)

legend("topright", c("Water", "Sediment"), col = c("skyblue", "wheat"),
         pt.bg = c("skyblue", "wheat"), pch = c(15,15), cex = 1.5, bty = "n")

dev.off()
graphics.off()
```

# Figure 4: Beta-diversity is higher among headwaters than among higher order streams.
```{r}
orders.bray.w <- as.data.frame(cbind(c(
        rep(1, length(order.1.dists.w)),
        rep(2, length(order.2.dists.w)),
...
boxplot(dist ~ order, data = orders.bray.w, at = c(1,4), xlim = c(0,6), ylim = c(0.4, 0.9), 
        col = "skyblue", xlab = "", xaxt = "n", yaxt = "n")
boxplot(dist ~ order, data = orders.bray.s, at = c(2,5), add = T, col = "wheat",
        xlab = "", xaxt = "n", yaxt = "n")
