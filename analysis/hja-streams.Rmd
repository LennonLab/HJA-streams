...
shi.env.dists <- vegdist(env.2[which(design$order >= 2),], method = "bray")
shi.env.dists <- liste(shi.env.dists, entry = "env")[,3]
shi.geo.dists <- na.omit(liste(dist.mat[which(design$order >= 2), which(design$order >= 2)],
                                 entry = "geo.dist"))
shi.db <- vegdist(OTUsREL[which(design$order >= 2),])
...
# shi.s.env.dists <- vegdist(env.2[which(design$order > 1 & design$habitat == "sediment"),], method = "bray")
# shi.s.env.dists <- liste(shi.s.env.dists, entry = "env")[,3]
# shi.s.geo.dists <- na.omit(liste(dist.mat[which(design$order > 1 & design$habitat == "sediment"), 
#                                           which(design$order > 1 & design$habitat == "sediment")],
#                                  entry = "geo.dist")[,3])
# shi.s.db <- vegdist(OTUsREL[which(design$order > 1 & design$habitat == "sediment"),])
# shi.s.dists <- cbind(liste((1 - shi.s.db), entry = "comm.struc"), shi.s.env.dists, shi.s.geo.dists)
# shi.s.lm <- (lm(log(shi.s.dists$comm.struc) ~ shi.s.dists$shi.s.env.dists))
# summary(shi.lm)
# plot(shi.s.dists$shi.s.env.dists, log(shi.s.dists$comm.struc))
# abline(shi.s.lm)
...
plot(s1.dists$geo.dist, 
     s1.dists$s1.env.dists)
abline(lm(s1.dists$s1.env.dists ~ s1.dists$geo.dist))
plot(shi.dists$geo.dist, shi.dists$shi.env.dists)
abline(lm(shi.dists$shi.env.dists ~ shi.dists$geo.dist))
```
