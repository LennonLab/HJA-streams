...
s.size <- as.vector(c(rowSums(OTUsREL.log)))
hja.alpha <- d(OTUsREL.log, lev = "alpha", q = 1, boot = T, 
               boot.arg = list(s.sizes = s.size))
hja.beta <- d(OTUsREL.log, lev = "beta", q = 1, boot = T, 
              boot.arg = list(s.sizes = s.size))
hja.gamma <- d(OTUsREL.log, lev = "gamma", q = 1, boot = T, 
               boot.arg = list(s.sizes = s.size))

water.alpha <- d(OTUsREL.log[which(design$habitat == "water"),], 
                 lev = "alpha", q = 1, boot = T, 
                 boot.arg = list(s.sizes = s.size[which(design$habitat == "water")]))
sediment.alpha <- d(OTUsREL.log[which(design$habitat == "sediment"),], 
                 lev = "alpha", q = 1, boot = T, 
...
water.env.spat.var <- vegan::rdaa(OTUsREL.log[which(design$habitat == "water"),] ~
...
plot(hja.dists$env.dists, log(1 - hja.dists$comm.struc))
plot(hja.dists$den.dists, log(1 - hja.dists$comm.struc))
plot(hja.dists$den.dists, hja.dists$env.dists)
plot(hja.dists$geo.dists, hja.dists$env.dists)
plot(hja.dists$geo.dists, log(1 - hja.dists$comm.struc))


abline(lm(log(1-hja.dists$comm.struc) ~ hja.dists$env.dists))
abline(lm(log(1-hja.dists$comm.struc) ~ hja.dists$den.dists))
abline(lm(log(1-hja.dists$comm.struc) ~ hja.dists$geo.dists))
...
plot(water.dists$water.env.dists, log(water.dists$comm.struc), xlab="", 
     ylab = "", xaxt="n", yaxt="n")
...
plot(sed.dists$sed.env.dists, log(sed.dists$comm.struc), xlab="", 
     ylab = "", xaxt = "n", yaxt = "n")
axis(side=1, labels=T, lwd.ticks=2, cex.axis=1.2, las=1)
axis(side=2, labels=T, lwd.ticks=2, cex.axis=1.2, las=1)
axis(side=3, labels=F, lwd.ticks=2, cex.axis=1.2, las=1)
...
```{r}
headwaters <- rownames(OTUs[which(design$order == 1),])

for(origin in headwaters){
  print(shortest_paths(graph = stream.network, from = origin, to = "LC_01_S")$vpath[[1]])
}

shortest_paths(graph = stream.network, from = "LC_11_W", to = "LC_01_S")$vpath[[1]]
```