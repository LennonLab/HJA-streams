...
abline(h = 0, v = 0, lty = 3)
box(lwd = 2)
points(hja.pcoa$points[which(design$order == 1),1], 
       hja.pcoa$points[which(design$order == 1),2],
       pch=19, cex=2, bg="lightskyblue", col="lightskyblue")
points(hja.pcoa$points[which(design$order == 2),1], 
       hja.pcoa$points[which(design$order == 2),2],
       pch=19, cex=2, bg="lightskyblue1", col="lightskyblue1")
points(hja.pcoa$points[which(design$order == 3),1], 
       hja.pcoa$points[which(design$order == 3),2],
       pch=19, cex=2, bg="lightskyblue2", col="lightskyblue2")
points(hja.pcoa$points[which(design$order == 4),1], 
       hja.pcoa$points[which(design$order == 4),2],
       pch=19, cex=2, bg="lightskyblue3", col="lightskyblue3")
points(hja.pcoa$points[which(design$order == 5),1], 
       hja.pcoa$points[which(design$order == 5),2],
       pch=19, cex=2, bg="lightskyblue4", col="lightskyblue4")
text(hja.pcoa$points[ ,1], hja.pcoa$points[ ,2],
     cex = 0.5, labels = row.names(hja.pcoa$points))
...
### Bray-Curtis Similarity by Elevation
```{r}
...
### Bray-Curtis Similarity by Stream Order
```{r}
...
order.1.bray.mean <- as.vector(round(apply(order.1.bray, 1, mean), 3))
order.2.bray.mean <- as.vector(round(apply(order.2.bray, 1, mean), 3))
order.3.bray.mean <- as.vector(round(apply(order.3.bray, 1, mean), 3))
order.4.bray.mean <- as.vector(round(apply(order.4.bray, 1, mean), 3))
order.5.bray.mean <- as.vector(round(apply(order.5.bray, 1, mean), 3))

order.1.bray.se <- as.vector(round(apply(order.1.bray, 1, se), 3))
order.2.bray.se <- as.vector(round(apply(order.2.bray, 1, se), 3))
order.3.bray.se <- as.vector(round(apply(order.3.bray, 1, se), 3))
order.4.bray.se <- as.vector(round(apply(order.4.bray, 1, se), 3))
order.5.bray.se <- as.vector(round(apply(order.5.bray, 1, se), 3))
...
plot(orders.bray$mean ~ orders.bray$order, xlab="Strahler Order",
...
### CCA
```{r, eval=FALSE}
hja.cca <- vegan::cca(OTUsREL.log ~ env.mat[,c(1:4,6)])
...
plot(scores(hja.cca, display = "wa"), xlim = c(-5, 5), ylim = c(-4, 2),
...
### CCA by Order
```{r}
env.mat <- as.matrix(env[,c(10:15)])

# Order 1
lc.1.cca <- vegan::cca(OTUsREL.log[
  which(design$watershed == "LC" & design$order == 1),],
  env.mat[which(design$watershed == "LC" & design$order == 1),]) 
#anova(hja.cca, by = "axis")
#cca.fit <- envfit(hja.cca, env.mat, perm = 999)
lc.1.cca.explainvar1 <- round(lc.1.cca$CCA$eig[1] /
                           sum(c(lc.1.cca$CCA$eig, lc.1.cca$CA$eig)), 3) * 100
lc.1.cca.explainvar2 <- round(lc.1.cca$CCA$eig[2] /
                           sum(c(lc.1.cca$CCA$eig, lc.1.cca$CA$eig)), 3) * 100

# Order 2
lc.2.cca <- vegan::cca(OTUsREL.log[
  which(design$watershed == "LC" & design$order == 2),],
  env.mat[which(design$watershed == "LC" & design$order == 2),]) 
#anova(hja.cca, by = "axis")
#cca.fit <- envfit(hja.cca, env.mat, perm = 999)
lc.2.cca.explainvar1 <- round(lc.2.cca$CCA$eig[1] /
                           sum(c(lc.2.cca$CCA$eig, lc.2.cca$CA$eig)), 3) * 100
lc.2.cca.explainvar2 <- round(lc.2.cca$CCA$eig[2] /
                           sum(c(lc.2.cca$CCA$eig, lc.2.cca$CA$eig)), 3) * 100

# Order 3
lc.3.cca <- vegan::cca(OTUsREL.log[
  which(design$watershed == "LC" & design$order == 3),],
  env.mat[which(design$watershed == "LC" & design$order == 3),]) 
#anova(hja.cca, by = "axis")
#cca.fit <- envfit(hja.cca, env.mat, perm = 999)
lc.3.cca.explainvar1 <- round(lc.3.cca$CCA$eig[1] /
                           sum(c(lc.3.cca$CCA$eig, lc.3.cca$CA$eig)), 3) * 100
lc.3.cca.explainvar2 <- round(lc.3.cca$CCA$eig[2] /
                           sum(c(lc.3.cca$CCA$eig, lc.3.cca$CA$eig)), 3) * 100

# Order 4
lc.4.cca <- vegan::cca(OTUsREL.log[
  which(design$watershed == "LC" & design$order == 4),],
  env.mat[which(design$watershed == "LC" & design$order == 4),]) 
#anova(hja.cca, by = "axis")
#cca.fit <- envfit(hja.cca, env.mat, perm = 999)
lc.4.cca.explainvar1 <- round(lc.4.cca$CCA$eig[1] /
                           sum(c(lc.4.cca$CCA$eig, lc.4.cca$CA$eig)), 3) * 100
lc.4.cca.explainvar2 <- round(lc.4.cca$CCA$eig[2] /
                           sum(c(lc.4.cca$CCA$eig, lc.4.cca$CA$eig)), 3) * 100

# Order 5
lc.5.cca <- vegan::cca(OTUsREL.log[
  which(design$watershed == "LC" & design$order == 5),],
  env.mat[which(design$watershed == "LC" & design$order == 5),]) 
#anova(hja.cca, by = "axis")
#cca.fit <- envfit(hja.cca, env.mat, perm = 999)
lc.5.cca.explainvar1 <- round(lc.5.cca$CCA$eig[1] /
                           sum(c(lc.5.cca$CCA$eig, lc.5.cca$CA$eig)), 3) * 100
lc.5.cca.explainvar2 <- round(lc.5.cca$CCA$eig[2] /
                           sum(c(lc.5.cca$CCA$eig, lc.5.cca$CA$eig)), 3) * 100

par(mar = c(5, 5, 4, 4) + 0.1)
plot(scores(lc.1.cca, display = "wa"), xlim = c(-2, 3), ylim = c(-2, 2),
     xlab = paste("CCA 1 (", lc.1.cca.explainvar1, "%)", sep = ""), 
     ylab = paste("CCA 2 (", lc.1.cca.explainvar2, "%)", sep = ""), 
     pch = 16, cex = 2.0, type = "n", cex.lab = 1.5, cex.axis = 1.2, axes = FALSE)
axis(side=1, labels=T, lwd.ticks=2, cex.axis=1.2, las=1)
axis(side=2, labels=T, lwd.ticks=2, cex.axis=1.2, las=1)
abline(h=0, v=0, lty=3)
box(lwd=2)
points(scores(lc.1.cca, display = "wa"),
       pch=19, cex=2, bg="gray", col="gray")
text(scores(lc.1.cca, display = "wa"), cex = 0.5,
     labels = row.names(scores(lc.1.cca, display = "wa")))
lc.1.cca.vectors <- scores(lc.1.cca, display = "bp")
row.names(lc.1.cca.vectors) <- c("elev", "temp")
arrows(0, 0, lc.1.cca.vectors[, 1] * 2, lc.1.cca.vectors[, 2]*2,
       lwd = 2, lty = 1, length = 0.2, col = "red")
text(lc.1.cca.vectors[, 1] * 2, lc.1.cca.vectors[, 2] * 2, pos=3,
     labels = row.names(lc.1.cca.vectors), col = "red")

par(mar = c(5, 5, 4, 4) + 0.1)
plot(scores(lc.2.cca, display = "wa"), xlim = c(-2, 3), ylim = c(-2, 2),
     xlab = paste("CCA 1 (", lc.2.cca.explainvar1, "%)", sep = ""), 
     ylab = paste("CCA 2 (", lc.2.cca.explainvar2, "%)", sep = ""), 
     pch = 16, cex = 2.0, type = "n", cex.lab = 1.5, cex.axis = 1.2, axes = FALSE)
axis(side=1, labels=T, lwd.ticks=2, cex.axis=1.2, las=1)
axis(side=2, labels=T, lwd.ticks=2, cex.axis=1.2, las=1)
abline(h=0, v=0, lty=3)
box(lwd=2)
points(scores(lc.2.cca, display = "wa"),
       pch=19, cex=2, bg="gray", col="gray")
text(scores(lc.2.cca, display = "wa"), cex = 0.5,
     labels = row.names(scores(lc.2.cca, display = "wa")))
lc.2.cca.vectors <- scores(lc.2.cca, display = "bp")
row.names(lc.2.cca.vectors) <- c("elev", "temp")
arrows(0, 0, lc.2.cca.vectors[, 1] * 2, lc.2.cca.vectors[, 2]*2,
       lwd = 2, lty = 1, length = 0.2, col = "red")
text(lc.2.cca.vectors[, 1] * 2, lc.2.cca.vectors[, 2] * 2, pos=3,
     labels = row.names(lc.2.cca.vectors), col = "red")
```
...
OTU.tax[OTU.tax$OTU %in% unique(as.vector(top.taxa[,1])),]
```
...
var.part.1 + var.part.4
var.part.2 + var.part.3
