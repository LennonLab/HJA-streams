...
simpsD <- diversity(OTUs, index = "invsimpson")
...
rared <- t(S.rarefy)
colnames(rared) <- c("S.rare", "se.rare")
...
alpha.div <- cbind(design, S.obs, simpsE, shan, shan2, simpsD, rared)
```
...
t.test(alpha.div$shan2 ~ alpha.div$habitat)
plot(alpha.div$shan2 ~ alpha.div$habitat)
...
```{r}
t.test(alpha.div$S.rare ~ alpha.div$habitat)

png(filename = "./figures/Figure1.png",
    width = 1200, height = 1200, res = 96*2)
par(mar = c(5, 5, 3, 2) + 0.3)
plot(alpha.div$S.rare ~ alpha.div$habitat, 
     names = c("Sediment", "Water"), 
     yaxt = "n", ylab = "", xlab = "", cex.axis = 1.2)
axis(side = 2, labels = T, lwd.ticks = 2, cex.axis = 1.2, las = 1)
axis(side = 1, labels = F, at = c(1,2), lwd.ticks = 2, cex.axis = 1.2, las = 1)
box(lwd = 2)
text(1, 2500, "*", cex = 2)
mtext("Habitat type", side = 1, line = 3, cex = 1.5)
mtext("Rarefied species richness", side = 2, line = 4, cex = 1.5)
dev.off()
graphics.off()
...
#### Figure 2: Sediments & Water Communities PCoA
```{r}
...
#### Figure 3: Surface Water vs. Sediment Communities
```{r}
...
```{r}
order.1.bray <- vegdist(OTUsREL.log[which(design$order==1),which(design$order==1)], method = "bray")
order.2.bray <- vegdist(OTUsREL.log[which(design$order==2),which(design$order==2)], method = "bray")
order.3.bray <- vegdist(OTUsREL.log[which(design$order==3),which(design$order==3)], method = "bray")
order.4.bray <- vegdist(OTUsREL.log[which(design$order==4),which(design$order==4)], method = "bray")
order.5.bray <- vegdist(OTUsREL.log[which(design$order==5),which(design$order==5)], method = "bray")

order.1.dists <- as.vector(unlist(list(order.1.bray)))
order.2.dists <- as.vector(unlist(list(order.2.bray)))
order.3.dists <- as.vector(unlist(list(order.3.bray)))
order.4.dists <- as.vector(unlist(list(order.4.bray)))
order.5.dists <- as.vector(unlist(list(order.5.bray)))

order.1.se <- se(order.1.bray)
order.2.se <- se(order.2.bray)
order.3.se <- se(order.3.bray)
order.4.se <- se(order.4.bray)
order.5.bray.se <- se(order.5.bray)

orders.bray <- as.data.frame(cbind(c(rep(1, length(order.1.dists)),
        rep(2, length(order.2.dists)),
        rep(3, length(order.3.dists)),
        rep(4, length(order.4.dists)),
        rep(5, length(order.5.dists))),
      c(order.1.dists, order.2.dists, order.3.dists,
        order.4.dists, order.5.dists)))
colnames(orders.bray) <- c("order", "dist")
model.orders <- lm(orders.bray$dist ~ orders.bray$order)
modsum <- summary(model.orders)
```

### Figure 4: No relationship between beta-diversity and stream order
```{r}
png(filename = "./figures/Figure4.png",
    width = 1200, height = 1200, res = 96*2)
par(mar = c(5, 5, 2, 2) + 0.1)
plot(orders.bray$dist ~ orders.bray$order, xaxt = "n", yaxt = "n",
...
boxplot(alpha.div$shan ~ alpha.div$habitat)
...
png(filename = "./figures/Figure5.png",
    width = 1200, height = 1200, res = 96*2)
par(mar = c(5, 5, 2, 2) + 0.1)
boxplot((1-water.frac), (1-sed.frac), names = c("Water", "Sediments"), yaxt = "n", cex.axis = 1.5)
axis(side = 2, labels = T, lwd.ticks = 2, cex.axis = 1.2, las = 1)
axis(side = 1, labels = F, at = c(1,2), lwd.ticks = 2, cex.axis = 1.2, las = 1)
box(lwd=2)
mtext("Proportion Unique Taxa", side = 2, line = 3.5, cex = 1.5)
text(x = 2, y = 0.25, "*", cex = 2)
dev.off()
graphics.off()

t.test(water.frac, sed.frac)
```

```{r, eval = F}
rowSums(in.sediment[which(design$habitat == "sediment"), which(colSums(in.sediment[which(design$habitat == "water"),]) == 0)]) / rowSums(in.sediment[which(design$habitat == "sediment"),])

pres.abs.list[which(design$habitat == "water"), 2] / pres.abs.list[which(design$habitat == "water"), 6]
pres.abs.list[which(design$habitat == "sediment"), 1] / pres.abs.list[which(design$habitat == "sediment"), 6]

plot(pres.abs.list[which(design$habitat == "water"),4],
     (pres.abs.list[which(design$habitat == "water"), 2] / pres.abs.list[which(design$habitat == "water"), 3]))

plot(pres.abs.list[which(design$habitat == "sediment"),4],
     (pres.abs.list[which(design$habitat == "sediment"), 1] / pres.abs.list[which(design$habitat == "sediment"), 3]))
```
...
plot((paired.div$S.rare[which(paired.div$habitat == "water")] -
        paired.div$S.rare[which(paired.div$habitat == "sediment")]) ~ unique(paired.div$site))
abline(h=0)
...
#### Figure 6: Water vs. Sed. diversity
```{r}
png(filename = "./figures/Figure6.png",
    width = 1200, height = 1200, res = 96*2)
par(c(5,5,3,2) + 0.3)
barplot(height = paired.div.difs, names.arg = unique(paired.div$site),
        horiz = TRUE, yaxt = "n", xaxt = "n", xlim = c(-2000,2000),
        col = "black", border = NA)
...
mtext(side = 3, line = 0.5, at = c(-2000), adj = 0, "Higher in\nsediment")
mtext(side = 3, line = 0.5, at = c(2000), adj = 1, "Higher in\nwater")
...
summary(model.water)
plot(water.sim$water.bray.mean ~ water.sim$elev, xlim = c(400,1300), ylim = c(.2,.5), xlab="",
     ylab = "", xaxt = "n", yaxt = "n")
axis(side=1, labels=F, lwd.ticks=2, cex.axis=1.2, las=1)
axis(side=2, labels=T, at = c(.2,.3,.4,.5), lwd.ticks=2, cex.axis=1.2, las=1)
axis(side=3, labels=F, lwd.ticks=2, cex.axis=1.2, las=1)
axis(side=4, labels=F, at = c(.2,.3,.4,.5), lwd.ticks=2, cex.axis=1.2, las=1)
...
summary(model.sed)
plot(sed.sim$sed.bray.mean ~ sed.sim$elev, 
     xlim = c(400,1300), ylim = c(.2,.5), xlab="",
     ylab = "", xaxt = "n", yaxt = "n")
axis(side=1, labels=T, lwd.ticks=2, cex.axis=1.2, las=1)
axis(side=2, labels=T, at = c(.2,.3,.4,.5), lwd.ticks=2, cex.axis=1.2, las=1)
axis(side=3, labels=F, lwd.ticks=2, cex.axis=1.2, las=1)
...
### Figure S4: Bray-Curtis Similarity by Elevation - Same Plot
```{r}
...
summary(model.water)
plot(water.sim$water.bray.mean ~ water.sim$elev, xlim = c(400,1300), ylim = c(.2,.5), xlab="",
     ylab = "", xaxt = "n", yaxt = "n", pch = 21)
points(sed.sim$sed.bray.mean ~ sed.sim$elev, pch = 20)
...
mtext("Bray-Curtis Similarity", side = 2, line = 3.5, cex = 1.5)
mtext("Elevation (m)", side = 1, line = 3, cex = 1.5)

abline(model.water, lty = 1)
abline(model.sed, lty = 2)

legend("bottomright", c("Water", "Sediment"), pch = c(21,20), cex = 1.5, bty = "n")
