...
dists.df <- simba::liste(RC.hel.dist, entry = "RC_hel") %>% add_column(den_dists = flow.connected.dists)
...
hja.assembly.plot <- hja.assembly %>% filter(den_dists > 0) %>% 
  mutate(process = ifelse(bNTI < -2, "Convergent species sorting", 
                          ifelse(bNTI > 2, "Divergent species sorting",
...
hja.assembly.grouped <- hja.assembly.plot %>% 
  mutate(den_dists_rounded = 10^round(log10(den_dists),0)) %>% 
...
hja.assembly.grouped %>% 
  ...
  mutate(process = factor(process, levels = c("Stochastic", 
                                              "Divergent species sorting",
                                              "Convergent species sorting",
                                              "Mass effects",
...
horizlines <- data.frame(metric = factor(levels(hja.assembly.plot$metric)[c(1,1,2,2)]), y = c(-1.96, 1.96, -0.95, 0.95))
...
ddrpanelplot <- hja.assembly %>% filter(den_dists > 0) %>% 
  ...
  geom_hline(aes(yintercept = 1.96), alpha = 0.25, size = .5, linetype = "dashed") +
  geom_hline(aes(yintercept = -1.96), alpha = 0.25, size = .5, linetype = "dashed") +
...
beta.summary <- subset(beta.to.plot, beta != "beta[\"RC, Hellinger distance\"]") %>% 
  group_by(position, habitat) %>% 
  summarise(mean = mean(dist), se = sd(dist)/n()) %>% 
  mutate(lower = mean - se, upper = mean + se)
beta.summary$habitat_labels <- c("S", "P", "S-P", "S", "P", "S-P")
...
beta.to.plot %>% filter(beta != "beta[\"RC, Hellinger distance\"]") %>% 
  ...
  facet_wrap(~beta, scales = "free_y", strip.position = "left", labeller = label_parsed) +
  theme(strip.placement = "outside") +
  labs(y = "", x = "") +
  scale_color_manual(values=colorspace::darken(c(my.colors, "#688c45"),0.4)) +
  scale_fill_manual(values=c(my.colors, "#688c45", my.colors,	"#688c45")) +
  theme(legend.position = "none") +
  geom_text(data = beta.summary, mapping = aes(label = habitat_labels, y = -16),
            position = position_dodge(width = .75), vjust = 1) +
  coord_cartesian(ylim = c(-16, 48), clip = "off") +
