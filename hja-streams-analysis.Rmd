...
alpha.tbl %>% 
  ggplot(aes(x = order, y = N1, color = habitat)) +
...
unique.fig <- unique.fracs %>% 
  ggplot(aes(x = habitat, y = unique_frac)) + 
  geom_boxplot() +
  geom_jitter(alpha = 0.2) +
  labs(x = "", y = "Habitat-specific taxa (richness)") +
  #scale_fill_manual(values = (my.colors)) +
  guides(fill = FALSE) +
...
# unique.fracs %>% 
#   ggplot(aes(x = order, y = unique_frac, color = habitat)) +
#   geom_point() + 
#   geom_smooth()
```
...
levels(alpha.to.plot$metric) <- c(
  expression(paste(alpha, "-diversity (species equivalents)")),
  expression(paste("Proportion habitat-specific taxa (richness)"))
)
alpha.to.plot %>% 
  ...
  ggsave("figures/Fig1.png", width = 6, height = 4, dpi = 600)
```
...
hja.assembly %>% filter(den_dists > 0) %>% 
  ggplot(aes(x = den_dists, y = RC_hel)) +
  geom_point() + 
  geom_smooth() + 
  facet_wrap(~habitat)
...
hja.assembly.plot <- hja.assembly %>% filter(den_dists > 0) %>% 
  ...
  mutate(signif = ifelse(abs(RC_hel) < 0.95 & abs(bNTI) < 1.96, FALSE, TRUE)) %>% 
...
hja.assembly.plot %>% mutate(scale = ifelse(den_dists < 1500, "Local", "Regional")) %>% 
  ...
  facet_grid(metric~., scales = "free", switch = "y", labeller = label_parsed) + 
  theme(strip.placement = "outside", 
  ...
  ggsave("figures/ddr-panel.png", width = 4, height = 4/3*4, dpi = 600)

# hja.assembly.plot %>% mutate(scale = ifelse(den_dists < 1000, "Local", "Regional")) %>% 
#   filter(metric == 'beta["RC, Hellinger distance"]') %>% 
#   ggplot(aes(x = value, color = habitat, fill = habitat)) +
#   geom_density(alpha = .2) + 
#   facet_grid(.~scale, scales = "free", switch = "y", labeller = label_parsed) + 
#   theme(strip.placement = "outside", 
#         legend.title = element_blank(),
#         legend.text = element_text(size = 9),
...
# 
# 
# hja.assembly.plot %>% mutate(den_dists = factor(floor(log10(den_dists)), ordered = T, levels = c(0,1,2,3,4))) %>% 
#   group_by(den_dists, habitat, process) %>% 
#   ggplot(aes(x = process, color = habitat, fill = habitat)) +
#   geom_bar(position = "dodge") +
#   facet_grid(den_dists ~ metric, scales = "free", switch = "y", labeller = label_parsed) +
#   theme(strip.placement = "outside", 
#         legend.title = element_blank(),
#         legend.text = element_text(size = 9),
#         legend.position = "bottom") +
...
# 
# library(nnet)
# hja.assembly.mod <- hja.assembly.plot 
# hja.assembly.mod$process <- factor(hja.assembly.mod$process)
# hja.assembly.mod$process <- relevel(hja.assembly.mod$process, ref = "Stochastic")
# hja.assembly.mod$habitat <- droplevels(hja.assembly.mod$habitat)
# mmod <- multinom(process ~ habitat + den_dists + env, data = hja.assembly.mod)
# summary(mmod)
# mmodi <- step(mmod)
# summary(mmodi)
# 
# library(MASS)
# mlda <- lda(process ~ habitat + den_dists + env, data = hja.assembly.mod)
# mlda
# 
# predict(mlda)
...
beta.to.plot$habitat <-  factor(beta.to.plot$habitat, levels = c("Sediment", "Planktonic"))
beta.to.plot %>% 
  ggplot(aes(x = position, y = dist, fill = habitat)) +
  ...
  scale_fill_manual(values = my.colors) + 
  theme(legend.position = "none") +
  ggsave("figures/headwater_mainstem.pdf", width = 6, height = 4) +
  ggsave("figures/headwater_mainstem.png", width = 6, height = 4, dpi = 600)



summary(lm(dist ~ position * habitat, data = subset(beta.to.plot, beta == "beta[\"RC, Hellinger distance\"]")))
summary(lm(dist ~ position * habitat, data = subset(beta.to.plot, beta == 'paste(beta, "NTI")')))
```
