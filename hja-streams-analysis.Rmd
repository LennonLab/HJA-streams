...
# Run a PERMANOVA
hja.permanova <- adonis(hja.pcoa$dist.matrix ~ design$habitat + design$order + design$watershed, permutations = 999)
...
hja.rda.reduced <- rda(OTUsREL.hel ~ habitat + elevation + conductivity + TP + TN + ph + DOC, env.subs)
```
...
```{r}
rda.plot <- cbind.data.frame(scores(hja.rda.reduced)$sites, group = str_to_title(design$habitat), watershed = design$watershed)
...
rda.vecs$origin <- 0
scale.arrows = .5
hja.rda.fig <-  ggplot(data = rda.plot, aes(x = RDA1, y = RDA2)) +
  geom_hline(aes(yintercept = 0), linetype = "dashed", alpha = 0.25, size = 0.25) +
  geom_vline(aes(xintercept = 0), linetype = "dashed", alpha = 0.25, size = 0.25) +
  ...
  #           aes(x = Dim1, y = Dim2, color = group),
  #           size = .8, alpha = 1, linetype = 2) +
  stat_ellipse(data = rda.plot, alpha = 0.8, aes(color = group, shape = watershed)) +
  ...
  geom_text_repel(data = rda.vecs, size = 3,
                   ...
                   color = "black",
                   segment.alpha = 0,
                  point.padding = 0, nudge_y = c(0, 0, .035, 0, 0, .05, -.05)) +
  #annotate("text", x = .3, y = -.25, label = "Sediment", size = 3) +
  #annotate("text", x = -.27, y = -.22, label = "Plankton", size = 3) +
  ggsave("figures/RDA.png", width = 8, height = 8, units = "cm", dpi = 500) +
  ggsave("figures/RDA.pdf", width = 8, height = 8, units = "cm")
```
...
obs.hel <- as.matrix(vegdist(OTUsREL, method = "bray"))
...
dists.df <- simba::liste(RC.hel.dist, entry = "RC_hel") %>% add_column(den_dists = flow_connected_dists_df$den_dists)
...
dists.df$habitat <- factor(dists.df$habitat, levels = c("Sediment", "Planktonic", "Planktonic-Sediment"))
dists.df %>% filter(den_dists > 0) %>% 
  ggplot(aes(x = den_dists/1000, y = RC_hel)) +
  geom_point(alpha = 0.25, aes(color = abs(RC_hel) > 0.95)) +
...
hja.rchel.dist.ls <- simba::liste(RC.hel.dist, entry = "RCbray")
...
hja.assembly.grouped %>% 
  ...
  geom_area(alpha = .9, position = "stack", color = "gray90") +
  ...
  scale_fill_manual(values = c("#02440c","#789400","#e4d565","#f3f5e7","#40becd")) + 
...
headwaters.dist.sed.tax <- as.dist(as.matrix(RC.hel.dist)[
  which(design$order == 1 & design$habitat != "water"), 
  which(design$order == 1 & design$habitat != "water")])
mainstem.dist.sed.tax <- as.dist(as.matrix(RC.hel.dist)[
  which(design$order != 1 & design$habitat != "water"), 
  which(design$order != 1 & design$habitat != "water")])
headwaters.dist.water.tax <- as.dist(as.matrix(RC.hel.dist)[
  which(design$order == 1 & design$habitat == "water"), 
  which(design$order == 1 & design$habitat == "water")])
mainstem.dist.water.tax <- as.dist(as.matrix(RC.hel.dist)[
  which(design$order != 1 & design$habitat == "water"), 
  which(design$order != 1 & design$habitat == "water")])
...
headwaters.dist.sed.water.tax <- as.matrix(RC.hel.dist)[
  which(design$order == 1 & design$habitat != "water"), 
  which(design$order == 1 & design$habitat == "water")]
downstream.dist.sed.water.tax <- as.matrix(RC.hel.dist)[
  which(design$order != 1 & design$habitat != "water"), 
  which(design$order != 1 & design$habitat == "water")]
...
hwsp$habitat <- "Sediment"
hwwp$habitat <- "Planktonic"
hwswp$habitat <- "Sediment-Planktonic"
hwst$habitat <- "Sediment"
hwwt$habitat <- "Planktonic"
hwswt$habitat <- "Sediment-Planktonic"

dssp$habitat <- "Sediment"
dswp$habitat <- "Planktonic"
dsswp$habitat <- "Sediment-Planktonic"
dsst$habitat <- "Sediment"
dswt$habitat <- "Planktonic"
dsswt$habitat <- "Sediment-Planktonic"
...
assembly.position.grouped %>% 
  ...
  geom_bar(alpha = .9, position = "stack", stat = "identity") +
  facet_wrap(~ habitat, ncol = 1) +
  scale_fill_manual(values = c("#02440c","#789400","#e4d565","#f3f5e7","#40becd")) + 
  labs(x = "", y = "Proportion") +
  theme(legend.position = "bottom", 
...
hja.ddr.fig <- hja.assembly %>% left_join(simba::liste(den.dists, entry = "unconnected_dists")) %>% 
  ggplot(aes(x = unconnected_dists +1, y = hel, color = habitat, fill = habitat, shape = habitat)) +
...
ddr_envdist_mod <- lm(hel ~ unconnected_dists * habitat + env, data = ddr_formod)
ddr_dist_mod <- lm(hel ~ unconnected_dists * habitat, data = ddr_formod)
AIC(ddr_envdist_mod)
AIC(ddr_dist_mod)
plot(ddr_envdist_mod)
summary(ddr_envdist_mod)
summary(ddr_dist_mod)
```
